			...
			set
			{
				...
				if (m_breakpoints != null)
				{
					for (int i = 0; i < m_breakpoints.Count; ++i)
					{
						try
						{
							(m_breakpoints[i] as CorBreakpoint).Activate(value);
						}
		...
		public CorBreakpoint CorBreakpoint
		{
			get
			{
				if (m_breakpoints == null)
					...
					return (CorBreakpoint)m_breakpoints[0];
			}
		}
