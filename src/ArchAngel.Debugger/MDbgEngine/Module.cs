...
namespace Microsoft.Samples.Debugging.MdbgEngine
{
	...
	public sealed class MDbgModule : MarshalByRefObject, IComparable, IDisposable
	{
		...
		public ISymbolReader SymReader
		{
			get
			{
				...
				if (!m_isSymReaderInitialized && !CorModule.IsDynamic && !CorModule.IsInMemory)
				{
					...
					try
					{
						m_symReader = (SymBinder as SymbolStore.ISymbolBinder2).
		...
		public bool UpdateSymbols(IStream symbolStream)
		{
			...
			ISymbolReader newSymReader = (SymBinder as SymbolStore.ISymbolBinder2).GetReaderFromStream(Importer.RawCOMObject, symbolStream);
