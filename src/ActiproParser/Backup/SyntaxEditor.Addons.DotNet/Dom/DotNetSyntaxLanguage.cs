				...
				switch (context.Type) {
					...
					case DotNetContextType.ThisMemberAccess: {
						...
						if ((members != null) && (members.Length > 0) && (
							...
							syntaxEditor.IntelliPrompt.ParameterInfo.ValidTextRange = context.ParameterTextRange;
							syntaxEditor.IntelliPrompt.ParameterInfo.CloseDelimiterCharacter = ((forIndexer == true) && (language != DotNetLanguage.VB) ? ']' : ')');

							// Update the parameter index
							syntaxEditor.IntelliPrompt.ParameterInfo.UpdateParameterIndex();

							// Add markup for the first item
							syntaxEditor.IntelliPrompt.ParameterInfo.Info.Add(context.ProjectResolver.GetQuickInfoForMember(language, 
								context, type, members[0], syntaxEditor.IntelliPrompt.ParameterInfo.ParameterIndex, true));

							// Fill the rest of the items with nulls (they will be filled later)
							for (int index = 1; index < members.Length; index++)
								syntaxEditor.IntelliPrompt.ParameterInfo.Info.Add(null);

							if (syntaxEditor.IntelliPrompt.ParameterInfo.Info.Count > 0) {
								// Store the member array in the context
								syntaxEditor.IntelliPrompt.ParameterInfo.Context = new ParameterInfoContext(context, type, members);
