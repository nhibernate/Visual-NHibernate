...
namespace Algorithm.Diff
{
	...
	{
		public readonly Range OriginalRange;
		public readonly Range ChangedRange;
    ...
    public class Merge : IEnumerable
    {
    	...
    	public Merge(IList original, IList[] changed)
    		: this(original, changed, null, null)
    	{
    	}
    	...
    	public static IList MergeLists(IList original, IList[] changed)
    	{
    		return MergeLists(original, changed, null, null);
    	}

    	public static IList MergeLists(IList original, IList[] changed, IComparer comparer, IHashCodeProvider hashcoder)
        {
			// Construct a merge object.
            Merge m = new Merge(original, changed, comparer, hashcoder); 
            ArrayList ret = new ArrayList();
            List<Range> newlines = new List<Range>();
			// Process the hunks in the merge.
            foreach (Hunk h in m)
            {
                newlines.Clear();
                ...
                // If one list has changes, take them
                    ...
                    ret.AddRange(newlines[0]);

                // Indicate conflict
                else
                    ret.Add(new Conflict(newlines.ToArray()));
            }
            return ret;
        }

		public const int UserIndex = 0;
		public const int NewGenIndex = 1;
    	public const int PrevGenIndex = 2;
