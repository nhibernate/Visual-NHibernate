...
{
	public class SQLiteDatabaseLoader : IDatabaseLoader
	{
		private readonly static ILog log = LogManager.GetLogger(typeof(SQLiteDatabaseLoader));
		private readonly string[] _unsupportedDataTypes = new[] { "" };
		private ISQLiteDatabaseConnector connector;

		public event PropertyChangedEventHandler PropertyChanged;

		public SQLiteDatabaseLoader(ISQLiteDatabaseConnector connector)
		{
		...
		public List<SchemaData> GetSchemaObjects()
		{
			...
			foreach (string tableName in fullTableNames)
				tableNames.Add(tableName);

			foreach (string viewName in fullViewNames)
				viewNames.Add(viewName);

			schemaDatas.Add(new SchemaData("", tableNames, viewNames));
		...
		public Database LoadDatabase(List<SchemaData> databaseObjectsToFetch, List<string> allowedSchemas)
		{
			...
			try
			{
				var db = new Database(connector.DatabaseName, DatabaseTypes.SQLite) { Loader = this };
				db.ConnectionInformation = DatabaseConnector.ConnectionInformation;
				((SQLiteDatabaseConnector)DatabaseConnector).FillData();
		...
		public IDatabaseConnector DatabaseConnector
		{
			...
			set
			{
				if (value is SQLiteDatabaseConnector == false)
					throw new ArgumentException("Cannot use a " + value.GetType() +
												" as a connector for a SQLiteDatabaseLoader");
				connector = (SQLiteDatabaseConnector)value;
			}
		}
		...
		public List<Table> GetTables(List<SchemaData> tablesToFetch)
		{
			...
			foreach (string tableNameEx in tableNames)
			{
				...
				string tableName = tableNameEx;// parts[0];
		...
		public List<Table> GetViews(List<SchemaData> viewsToFetch)
		{
			...
			foreach (string viewNameEx in viewNames)
			{
				...
				string viewName = viewNameEx;// parts[0];
				string schema = "";// parts[1];
