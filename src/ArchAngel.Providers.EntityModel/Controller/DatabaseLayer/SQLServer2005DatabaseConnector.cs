...
{
	public interface ISQLServer2005DatabaseConnector : IDatabaseConnector
	{
	...
	public class SQLServer2005DatabaseConnector : ISQLServer2005DatabaseConnector
	{
		...
		private static readonly ILog log = LogManager.GetLogger(typeof(SQLServer2005DatabaseConnector));

		public SQLServer2005DatabaseConnector(ConnectionStringHelper connectionStringHelper)
		{
			try
			{
				string connectionString = connectionStringHelper.GetConnectionStringSqlClient(DatabaseTypes.SQLServer2005);
		...
		private SQLServer2005DatabaseConnector(string connectionString)
		{
			try
			{
				sqlConnection = new SqlConnection(connectionString);
				DatabaseName = sqlConnection.Database;
				ConnectionInformation = new ConnectionStringHelper(connectionString, DatabaseTypes.SQLServer2005);
			}
			catch (Exception e)
			{
				throw new DatabaseLoaderException("Could not open database connection. See inner exception for details.", e);
		...
		public static SQLServer2005DatabaseConnector FromConnectionString(string connectionString)
		{
			return new SQLServer2005DatabaseConnector(connectionString);
		}
		...
		public DataTable GetForeignKeyColumns()
		{
			if (foreignKeyColumns == null)
			{
				const string sql =
					@"
				SELECT
					CONSTRAINT_NAME, COLUMN_NAME, ORDINAL_POSITION, CONSTRAINT_SCHEMA, TABLE_SCHEMA, TABLE_NAME
				FROM
				...
				if (_indexes == null)
				{
					...
					const string sql2 = @"
						...
						FROM sysindexes i
							...
							INNER JOIN sysusers su on su.uid = o.uid
		...
		public List<string> GetTableNames()
		{
			...
			try
			{
				...
				{
					if (ex.Message.Contains("SCHEMA_NAME") || ex.Message.Contains("sys.extended_properties"))
			...
			if (foreignKeyConstraints == null)
			{
				const string sql =
					...
					On R.UNIQUE_CONSTRAINT_NAME = I.INDEX_NAME AND
						R.UNIQUE_CONSTRAINT_SCHEMA = I.CONSTRAINT_SCHEMA
					Order By ForeignKeyName
					";

				const string sql2000 = @"SELECT DISTINCT
						R.UNIQUE_CONSTRAINT_NAME as PrimaryKeyName,
						I.TABLE_NAME as PrimaryTable,
						I.TABLE_SCHEMA AS PrimaryTableSchema,
						R.CONSTRAINT_NAME as ForeignKeyName,
					...
					(
						SELECT		
							...
							TC.CONSTRAINT_TYPE AS CONSTRAINT_TYPE,			
							C.DATA_TYPE AS DATA_TYPE,
							CAST(CASE WHEN INDEXPROPERTY(OBJECT_ID(sysobjects.name),i.name,'IsClustered') = 1 THEN 1 ELSE 0 END AS BIT) AS IS_CLUSTERED,
							CAST(CASE WHEN INDEXPROPERTY(OBJECT_ID(sysobjects.name),i.name,'IsUnique') = 1 THEN 1 ELSE 0 END AS BIT) AS IS_UNIQUE
						FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE CU
							INNER JOIN INFORMATION_SCHEMA.COLUMNS C ON C.COLUMN_NAME = CU.COLUMN_NAME AND 
																	   C.TABLE_NAME = CU.TABLE_NAME AND
																	   C.TABLE_SCHEMA = CU.TABLE_SCHEMA
							INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS TC ON TC.TABLE_NAME = CU.TABLE_NAME AND 
																			TC.CONSTRAINT_NAME = CU.CONSTRAINT_NAME AND
																			TC.TABLE_SCHEMA = CU.TABLE_SCHEMA
							LEFT JOIN sysindexes AS i ON (i.id > 99 AND i.status & 0x20 = 0) AND i.name = CU.CONSTRAINT_NAME
				...
				{
					foreignKeyConstraints = RunQueryDataTable(sql2000);
				}
			}
