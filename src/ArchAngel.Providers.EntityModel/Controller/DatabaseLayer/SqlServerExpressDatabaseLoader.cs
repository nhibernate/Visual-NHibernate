...
{
	public class SQLServerExpressDatabaseLoader : IDatabaseLoader
	{
		private readonly static ILog log = LogManager.GetLogger(typeof(SQLServerExpressDatabaseLoader));
		...
		public Database LoadDatabase(List<SchemaData> databaseObjectsToFetch, List<string> allowedSchemas)
		{
			...
			try
			{
				var db = new Database(connector.DatabaseName, DatabaseTypes.SQLServerExpress) { Loader = this };
		...
		public IDatabaseConnector DatabaseConnector
		{
			...
			set
			{
				if (value is SQLServerExpressDatabaseConnector == false)
					throw new ArgumentException("Cannot use a " + value.GetType() +
												" as a connector for a SQLServerExpressDatabaseLoader");
				connector = (SQLServerExpressDatabaseConnector)value;
			}
		}
