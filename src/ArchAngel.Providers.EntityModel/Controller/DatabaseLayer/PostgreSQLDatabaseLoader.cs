...
{
	public class PostgreSQLDatabaseLoader : IDatabaseLoader
	{
		private readonly static ILog log = LogManager.GetLogger(typeof(PostgreSQLDatabaseLoader));
		private readonly string[] _unsupportedDataTypes = new[] { "" };
		private IPostgreSQLDatabaseConnector connector;

		public event PropertyChangedEventHandler PropertyChanged;

		public PostgreSQLDatabaseLoader(IPostgreSQLDatabaseConnector connector)
		{
		...
		public Database LoadDatabase(List<SchemaData> databaseObjectsToFetch, List<string> allowedSchemas)
		{
			...
			try
			{
				var db = new Database(connector.DatabaseName, DatabaseTypes.PostgreSQL) { Loader = this };
				db.ConnectionInformation = DatabaseConnector.ConnectionInformation;
				SetSchemaFilter(databaseObjectsToFetch);

				foreach (var table in GetTables(databaseObjectsToFetch, allowedSchemas))
					db.AddTable(table);

				foreach (var view in GetViews(databaseObjectsToFetch, allowedSchemas))
					db.AddView(view);
		...
		public IDatabaseConnector DatabaseConnector
		{
			...
			set
			{
				if (value is PostgreSQLDatabaseConnector == false)
					throw new ArgumentException("Cannot use a " + value.GetType() +
												" as a connector for a PostgreSQLDatabaseLoader");
				connector = (PostgreSQLDatabaseConnector)value;
			}
		}
		...
		private void GetUniqueIndexes(Table table)
		{
			...
			for (int i = 0; i < indexRows.Length; i++)
			{
				...
				Index index = new Index
				{
					...
					IsUserDefined = false,
					IsUnique = true,
					IsClustered = (int)indexRow["IS_CLUSTERED"] == 1,
