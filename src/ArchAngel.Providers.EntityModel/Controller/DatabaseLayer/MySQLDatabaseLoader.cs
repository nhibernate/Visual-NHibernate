...
{
	public class MySQLDatabaseLoader : IDatabaseLoader
	{
		private readonly static ILog log = LogManager.GetLogger(typeof(MySQLDatabaseLoader));
		private readonly string[] _unsupportedDataTypes = new[] { "" };
		private IMySQLDatabaseConnector connector;

		public event PropertyChangedEventHandler PropertyChanged;

		public MySQLDatabaseLoader(IMySQLDatabaseConnector connector)
		{
		...
		public IDatabaseConnector DatabaseConnector
		{
			...
			set
			{
				if (value is MySQLDatabaseConnector == false)
					throw new ArgumentException("Cannot use a " + value.GetType() +
												" as a connector for a MySQLDatabaseLoader");
				connector = (MySQLDatabaseConnector)value;
			}
		}
