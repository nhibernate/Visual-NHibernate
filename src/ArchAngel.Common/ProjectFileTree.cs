...
namespace ArchAngel.Common
{
	...
	public class ProjectFileTree : ProjectFileTreeNode
	{
		...
		private static readonly ILog log = LogManager.GetLogger(typeof(ProjectFileTree));
		...
		public override void RaiseNodeChangedEvent(ProjectFileTreeNode node, bool childrenChanged)
		{
		...
		/// </summary>
		public void RaiseTreeNeedsAnalysisEvent()
		{
		...
		public bool AllNodesAnalysed(bool overwriteExistingFiles)
		{
			foreach (ProjectFileTreeNode node in allNodes)
			{
				if (node.IsFolder)
					continue;
				if (node.Status == ProjectFileStatusEnum.UnAnalysedFile && !overwriteExistingFiles)
					return false;
		...
		public bool AllChangesResolved(bool overwriteExistingFiles)
		{
			foreach (ProjectFileTreeNode node in allNodes)
			{
				if (node.IsFolder)
		...
		public ProjectFileTreeNode CreateFileNodeForPath(string path)
		{
			var pathParts = path.Split(System.IO.Path.DirectorySeparatorChar);

			ProjectFileTreeNode newNode = this;
			for (int i = 0; i < pathParts.Length; i++)
			{
				var part = pathParts[i];
				if (part == ".") continue;
				var possibleChild = newNode.ChildNodes.FirstOrDefault(child => child.Text == part);

				// Only create a new node if we can't find one already named that.
				if (possibleChild != null)
				{
					newNode = possibleChild;
				}
				else
				{
					newNode = newNode.AddChildNode(part);
				}
				newNode.IsExternalToOutputFolder = System.IO.Path.IsPathRooted(path);
	...
	public class ProjectFileTreeNode
	{
		...
		private static readonly ILog log = LogManager.GetLogger(typeof(ProjectFileTreeNode));
		...
		/// <summary>
		/// Gets whether the path is underneath the output folder, or external to it.
		/// </summary>
		public virtual bool IsExternalToOutputFolder { get; internal set; }
		...
		public string Path
		{
			get
			{
				...
				{
					return System.IO.Path.Combine(parentNode.Path, Text);
				}
				return Text;
			}
		}
		...
		public virtual bool NodeSelected
		{
			...
			set
			{
				...
				{
					childNodes[i].NodeSelected = value;
				}
				RaiseNodeChangedEvent(this, true);
			}
		}
		...
		/// <param name="childrenChanged">Whether the children of this node also changed.</param>
		/// <param name="node">The node that changed.</param>
		public virtual void RaiseNodeChangedEvent(ProjectFileTreeNode node, bool childrenChanged)
		{
			if (parentTree != null)
			{
