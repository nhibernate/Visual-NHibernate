...
namespace ArchAngel.Workbench.IntelliMerge
{
	...
	public class QueueingTaskProgressHelper<T> : ITaskProgressHelper<T> where T : class
	{
		...
		public IEnumerable<ProgressMessage<T>> DequeueAllItems()
		{
			lock(lockObj)
			{
				foreach(ProgressMessage<T> item in queuedProgressMessages)
				{
					yield return item;
				}
	...
	public class AnalyseFilesProgress
	{
	    ...
	    public AnalyseFilesProgress(int numberOfFilesLeftToAnalyse, int numExactCopies, int numResolved, int numConflicts, int numErrors) : this(numberOfFilesLeftToAnalyse, numExactCopies, numResolved, numConflicts, numErrors, null, ProjectFileStatusEnum.UnAnalysedFile)
	    {
	    }
