...
using ArchAngel.Providers.CodeProvider.VB;
...
{
	///<summary>
	/// Used to create a VB.Net CodeRoot from VB.Net source code.
	///</summary>
	public class VBParser : ISemanticParseDataTarget, IParser
	{
		...
		private readonly VBFormatSettings formatSettings = new VBFormatSettings();
		private string currentFilename = "";
		private VBController controller = new VBController();
		private Document document;
		//private string currentCode = "";
		private Guid parserGuid = Guid.NewGuid();
		private VBCodeFormatter formatter;
		...
		/// </summary>
		public VBFormatSettings FormatSettings
		{
		...
		/// <summary>
		/// Parses the given code and creates a VB.Net CodeRoot from it.
		/// </summary>
		...
		public void ParseCode(string filename, string code)
		{
			...
			ParseCodeAsync(filename, code).WaitOne();
		}
		...
		private void CreateRegions(CompilationUnit compilationUnit)
		{
			...
			foreach (ActiproSoftware.SyntaxEditor.TextRange regionRange in compilationUnit.RegionTextRanges)
			{
				...
				while (regionStartToken != null)
				{
					...
					if (regionStartToken.ID == VBTokenID.Whitespace && tokenText == "\n")
					{
						break;
					}
		...
		//            commentValue = Slyce.Common.Utility.StandardizeLineBreaks(commentValue, Environment.NewLine);
		...
		//{
		//    string newComment = Slyce.Common.Utility.StandardizeLineBreaks(comment.Value.Text.Trim(), Environment.NewLine);
		...
		private readonly string MemberDeclarationStart = "Public Class Class1" + Environment.NewLine;
		private readonly string MemberDeclarationEnd = Environment.NewLine + "End Class";
		private readonly string InterfaceMemberDeclarationStart = "Public Interface Interface1" + Environment.NewLine;
		private readonly string InterfaceMemberDeclarationEnd = Environment.NewLine + "End Interface";
		private readonly string EnumMemberStart = "Public Enum Enum1" + Environment.NewLine;
		private readonly string EnumMemberEnd = Environment.NewLine + "End Enum";
		...
		public BaseConstruct ParseSingleConstruct(string code, BaseConstructType declaration)
		{
			...
			finally
			{
				...
				{
					throw new ParserException(
							"There was one or more errors while parsing the given code:" + Environment.NewLine + GetFormattedErrors());
				}
			}
		}
